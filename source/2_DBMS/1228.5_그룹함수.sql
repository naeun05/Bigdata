...
-- 부서 번호별 급여 최대치 
SELECT DEPTNO 부서번호, MAX(SAL) FROM EMP 
    GROUP BY DEPTNO ORDER BY 부서번호; -- GROUP BY 절에는 필드의 별칭이 올 수 없다.
-- 그룹함수는 숫자형, 문자형, 날짜형 모두 사용 가능 
-- 부서별로 최초 입사 년도, 최근 입사 년도 
SELECT DEPTNO, MIN(HIREDATE), MAX(HIREDATE) FROM EMP
    GROUP BY DEPTNO;
SELECT MIN(ENAME), MAX(ENAME) FROM EMP;
SELECT COUNT(JOB) FROM EMP; -- JOB 필드의 개수 ;14
SELECT COUNT(DISTINCT(JOB)) FROM EMP; -- JOB의 개수 ;5
...
-- 80년12월17일 최초입사  :14,620일째 83년01월12일 최근입사 :13,864일째
SELECT MIN(HIREDATE)||' 최초입사 :'|| TO_CHAR(TRUNC(SYSDATE-MIN(HIREDATE)),'99,999')|| '일째',
     MAX(HIREDATE) || ' 최근입사 :'|| TO_CHAR(TRUNC(SYSDATE-MAX(HIREDATE)),'99,999')||'일째' FROM EMP;
...
-- 부서번호별 사원 수 , 최대 급여, 최소 급여, 평균 급여
SELECT DEPTNO, COUNT(*),MAX(SAL),MIN(SAL),TRUNC(AVG(SAL)) 
    FROM EMP GROUP BY DEPTNO;
-- 부서명별 사원수, 최대 급여, 최소 급여, 평균 급여
...
-- 3. HAVING 조건(그룹함수 결과의 조건)
-- DEPTNO별로 평균급여 (평균 급여가 2000이상인 부서만 출력
...
SELECT DEPTNO,COUNT(*), ROUND(AVG(SAL)) AVG, MIN(SAL), MAX(SAL), SUM(SAL) FROM EMP GROUP BY DEPTNO ORDER BY SUM(SAL) DESC;

-- 5. 부서별, 업무별 그룹하여 결과를 부서번호, 업무, 인원수, 급여의 평균, 급여의 합을 
    --구하여 출력하라(출력결과는 부서번호, 업무순으로 오름차순 정렬)
SELECT DEPTNO, JOB, COUNT(*), AVG(SAL), SUM(SAL) FROM EMP E 
    GROUP BY DEPTNO,JOB
    ORDER BY DEPTNO, JOB;
...
SELECT E.DEPTNO,COUNT(*) FROM EMP E, DEPT D 
    WHERE E.DEPTNO=D.DEPTNO 
    GROUP BY E.DEPTNO HAVING COUNT(*)>=5;
...
--12. 위의 11번을 수정하여, 부서별 급여평균 최대치, 부서별 급여합의 최대치, 
            --부서별 최소급여의 최대치를 출력하라.  ****
SELECT MAX(ROUND(AVG(SAL))), MAX(SUM(SAL)), MAX(MIN(SAL)) FROM EMP E, DEPT D WHERE E.DEPTNO=D.DEPTNO GROUP BY DNAME;      
...
SELECT SUBSTR(HIREDATE,1,2) H_YEAR, COUNT(*), MIN(SAL), MAX(SAL), AVG(SAL), SUM(SAL) 
    FROM EMP GROUP BY SUBSTR(HIREDATE,1,2) ORDER BY H_YEAR ;
...
SELECT ENAME, SAL,
    RANK() OVER(ORDER BY SAL DESC) "RANK",   -- 데이터가 많을 경우 속도가 매우 느려서 실무에서 잘 안 씀
