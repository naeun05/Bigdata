...
-- 이름, 급여,상여(COMM), 연봉 ((급여+COMM)*12), 부서명, 근무지 
-- JOB이 SALESMAN 또는 MANAGER인 사원만 (연봉이 큰 순 정렬)
SELECT ENAME, SAL, COMM, (SAL+NVL(COMM,0))*12 연봉, DNAME, LOC FROM EMP E, DEPT D WHERE E.DEPTNO=D.DEPTNO AND UPPER(JOB) IN('SALESMAN','MANAGER')
    ORDER BY 연봉 DESC;
-- COMM이 NULL이고 급여가 1200이상인 사원의 이름, 급여, 입사일, 부서번호, 부서명 (부서명순, 급여큰순 정렬)
...
-- 모든 사원의 사번, 이름, JOB, 상사 사번, 급여, 급여등급(1등급..2등급..)
...
SELECT ENAME, SAL, GRADE, (SAL+NVL(COMM,0))*12 연봉, DNAME FROM EMP E, DEPT D, SALGRADE WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL 
    ORDER BY DNAME, 연봉;
...
SELECT W.ENAME || '의 매니저는 '|| NVL(M.ENAME,'無') ||'입니다.' FROM EMP W, EMP M WHERE W.MGR=M.EMPNO(+);

-- ★ <총 연습문제> 
-- PART1
--1. 이름, 직속상사
SELECT W.ENAME, M.ENAME 상사 FROM EMP W, EMP M WHERE W.MGR=M.EMPNO;

--2. 이름, 급여, 업무, 직속상사
SELECT W.ENAME, W.SAL, W.JOB, M.ENAME 상사 FROM EMP W, EMP M WHERE W.MGR=M.EMPNO;
...
--4. 이름, 급여, 부서명, 직속상사명
SELECT W.ENAME, W.SAL, DNAME, M.ENAME 상사 FROM EMP W, EMP M, DEPT D WHERE W.MGR=M.EMPNO AND W.DEPTNO=D.DEPTNO;

--5. 이름, 급여, 부서코드, 부서명, 근무지, 직속상사명, (상사가 없는 직원까지 전체 직원 다 출력)
SELECT W.ENAME, W.SAL, W.DEPTNO, DNAME, LOC, M.ENAME 상사 FROM EMP W, EMP M, DEPT D WHERE W.MGR=M.EMPNO(+) AND W.DEPTNO=D.DEPTNO;

--6. 이름, 급여, 등급, 부서명, 직속상사명. 급여가 2000이상인 사람
SELECT W.ENAME, W.SAL, GRADE, DNAME, M.ENAME 상사 FROM EMP W, EMP M, SALGRADE,DEPT D WHERE W.MGR=M.EMPNO AND W.SAL BETWEEN LOSAL AND HISAL
    AND W.DEPTNO=D.DEPTNO AND W.SAL>=2000;
    
--7. 이름, 급여, 등급, 부서명, 직속상사명, (직속상사가 없는 직원까지 전체직원 부서명 순 정렬) 
SELECT W.ENAME, W.SAL, GRADE, DNAME, M.ENAME 상사 FROM EMP W, EMP M, SALGRADE, DEPT D WHERE W.SAL BETWEEN LOSAL AND HISAL
    AND W.DEPTNO=D.DEPTNO AND W.MGR=M.EMPNO(+) ORDER BY DNAME;

--8. 이름, 급여, 등급, 부서명, 연봉, 직속상사명. 연봉=(급여+comm)*12 단 comm이 null이면 0
SELECT W.ENAME, W.SAL, GRADE, DNAME,(W.SAL+NVL(W.COMM,0))*12 연봉,M.ENAME 상사 FROM EMP W, EMP M, SALGRADE, DEPT D 
...
SELECT W.ENAME, M.ENAME 상사,K.ENAME 상사의상사 FROM EMP W, EMP M, EMP K WHERE W.MGR=M.EMPNO AND M.MGR=K.EMPNO; 
